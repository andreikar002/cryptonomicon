  ...
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    <div class="container">
        ...
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div class="mt-1 relative rounded-md shadow-md">
              ...
              />
              <dl class="mt-1 flex flex-row justify-around">
                <button
                  v-for="(hint, idx) in hints"
                  :key="idx"
                  class="flex items-center justify-center font-medium mr-2"
                  @click="add(hint.Symbol)"
                >
                  {{ hint.Symbol }}
                </button>
              </dl>
              <p v-if="isBeen" class="text-red-500">Такой тикер уже добавлен</p>
            </div>
          </div>
        </div>
        <button
      ...
      <template v-if="filteredTickers.length == 0">
        ...
        <div class="flex">
          <p class="mr-1">К сожалению,</p>
          <p v-if="filter" class="mr-1">таких</p>
          <p class="mr-1">тикеров нет</p>
        </div>
        <hr class="w-full border-t border-gray-600 my-4" />
      </template>
      <template v-if="filteredTickers.length > 0">
...
export default {
  ...
  async created() {
    ...
    this.listTickers = Object.values(data.Data);
    this.hints = this.listTickers.filter(
      (t) =>
        t.Symbol === "BTC" ||
        t.Symbol === "BTH" ||
        t.Symbol === "ETH" ||
    ...
    if (tickerData) {
      ...
      this.tickers.forEach((ticker) => this.tickersName.push(ticker.name));
    }
  },
  ...
  watch: {
    ...
    ticker() {
      ...
      this.isBeen = false;
      this.hints = this.listTickers
        .filter((t) => t.Symbol.indexOf(ticker) > -1)
        .sort((a, b) => {
          a = a.Symbol;
          b = b.Symbol;
          if (a.indexOf(ticker) > b.indexOf(ticker)) return 1;
          if (a.indexOf(ticker) < b.indexOf(ticker)) return -1;
          if (a.indexOf(ticker) == b.indexOf(ticker))
            return a.length - b.length;
        });
      if (this.hints.length > 4) this.hints = this.hints.slice(0, 4);
    },
  ...
  methods: {
    subscribeToUpdate(tickerName) {
      setInterval(async () => {
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD&api_key=7ac05d341f72c4af8201eb979f964464ef7158ada74effcb78f560bd3c42c9e4`
        );
        const data = await f.json();
        if (data.USD) {
    ...
    add(tickerName) {
      ...
      if (this.tickersName.includes(ticker)) {
        this.isBeen = true;
      } else {
        this.tickersName.push(ticker);
        const currentTicker = {
          name: ticker,
          price: "-",
        };
    ...
    handleDelete(tickerToRemove) {
      ...
      this.tickersName = this.tickersName.filter(
